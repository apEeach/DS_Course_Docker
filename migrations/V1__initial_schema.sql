...
CREATE TABLE IF NOT EXISTS `orders` (
    ...
    `created_by_staff_id`   BIGINT UNSIGNED DEFAULT NULL COMMENT '线下订单创建人',
    `created_at`            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at`            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `registrations` (
    ...
    KEY `idx_customer_id` (`customer_id`),
    KEY `idx_session_id` (`session_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='报名汇总';
...
CREATE TABLE IF NOT EXISTS `refunds` (
    ...
    `applicant_customer_id`  BIGINT UNSIGNED DEFAULT NULL COMMENT '申请客户ID',
    `source`                 ENUM('customer','staff') NOT NULL COMMENT '退款来源',
    `type`                   ENUM('full','partial') NOT NULL COMMENT '退款类型',
    ...
    KEY `idx_order_id` (`order_id`),
    KEY `idx_applicant` (`order_id`,`applicant_customer_id`,`created_at`),
    KEY `idx_status` (`status`)
...
CREATE TABLE IF NOT EXISTS `seat_assignments` (
    `id`                   BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `registration_slot_id` BIGINT UNSIGNED NOT NULL COMMENT '名额ID',
...
CREATE TABLE IF NOT EXISTS `checkin_records` (
    ...
    `source`               ENUM('scan','manual') NOT NULL COMMENT '签到来源',
    PRIMARY KEY (`id`),
